      ...
      <Card className="border-indigo-500/30 bg-gradient-to-r from-indigo-900/20 to-purple-900/20">
        <CardContent className="p-4">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="font-semibold text-indigo-300 text-sm">Modo Demo — Explora todas las funcionalidades</span>
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-2">
            {[
              { href: '/dashboard', label: 'Dashboard', emoji: '📊' },
              { href: '/dashboard/mente', label: 'Meditación IA', emoji: '🧠' },
              { href: '/dashboard/cuerpo', label: 'Nutrición IA', emoji: '💚' },
              { href: '/dashboard/entorno', label: 'Cultivos', emoji: '🌱' },
              ...
              { href: '/dashboard/configuracion', label: 'Config', emoji: '⚙️' },
            ].map((item) => (
              <Link
                key={item.href}
                href={item.href}
                className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-md bg-slate-800/60 hover:bg-slate-700/60 border border-slate-700/50 hover:border-indigo-500/40 transition-colors text-xs text-slate-300 hover:text-white"
              >
